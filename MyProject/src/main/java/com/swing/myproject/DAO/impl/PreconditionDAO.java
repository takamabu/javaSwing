...
public class PreconditionDAO{
    ...
    public PreconditionDAO() {
        this.column = Constant.PRECONDITION;
    }
    
    public List<PreconditionModel> findAll(){
        List<PreconditionModel> result = new ArrayList<>();
        int maxRow = Connection.getInstance().getSheet().getLastRowNum();
        for (int i = 2; i <= maxRow; i++){
            result.add(new PreconditionMapper(i, this.column).mapRow());            
        }
        return result;
    }
